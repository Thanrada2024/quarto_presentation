...
<!-- ═══ SLIDE 6a: ที่มา ═══ -->
## 📋 ข้อเสนอโครงการ — ที่มา {background-color="#0A1628"}
...
<div style="font-family:'Inter','Sarabun',sans-serif;font-size:0.62em;height:100%;display:flex;flex-direction:column;gap:0.3em;overflow:hidden;">

  <!-- Header -->
  <div style="background:linear-gradient(135deg,rgba(0,188,212,0.1),rgba(124,77,255,0.07));border:1px solid rgba(0,188,212,0.2);border-radius:9px;padding:0.35em 0.8em;display:flex;justify-content:space-between;align-items:center;flex:0 0 auto;">
    <div>
  ...
  <!-- Content -->
  <div style="flex:1;display:flex;gap:0.45em;overflow:hidden;min-height:0;">
    ...
    <div style="width:42%;display:flex;flex-direction:column;gap:0.25em;overflow:hidden;">
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.25em;flex:0 0 auto;">
        <div style="border:1.5px solid rgba(30,136,229,0.3);border-radius:7px;overflow:hidden;background:#060e1e;">
          <div style="height:160px;overflow:hidden;"><img src="img/etda_gov.png" style="width:100%;height:100%;object-fit:cover;object-position:top;display:block;" onerror="this.style.display='none'"></div>
          <div style="padding:0.18em 0.35em;border-top:1px solid rgba(30,136,229,0.2);">
            ...
            <div style="font-size:0.58em;color:#FFA726;background:rgba(255,167,38,0.1);border-radius:3px;padding:0.05em 0.25em;display:inline-block;margin-top:0.06em;">⚠️ ยังไม่มีกลไก Auto Enforce</div>
          </div>
        </div>
        <div style="border:1.5px solid rgba(124,77,255,0.3);border-radius:7px;overflow:hidden;background:#060e1e;">
          <div style="height:160px;overflow:hidden;"><img src="img/doc_center.png" style="width:100%;height:100%;object-fit:cover;object-position:top;display:block;" onerror="this.style.display='none'"></div>
          <div style="padding:0.18em 0.35em;border-top:1px solid rgba(124,77,255,0.2);">
            ...
            <div style="font-size:0.58em;color:#EF5350;background:rgba(239,83,80,0.1);border-radius:3px;padding:0.05em 0.25em;display:inline-block;margin-top:0.06em;">❌ ไม่อัพเดท</div>
          </div>
        </div>
      </div>

      <div style="flex:1;background:rgba(6,14,30,0.7);border:1px solid rgba(0,188,212,0.12);border-radius:8px;padding:0.3em 0.4em;display:flex;flex-direction:column;gap:0.14em;overflow:hidden;">
        <div style="display:flex;justify-content:center;flex:0 0 auto;">
          <div style="background:rgba(255,179,0,0.09);border:1.5px solid rgba(255,179,0,0.4);border-radius:6px;padding:0.13em 0.6em;text-align:center;position:relative;">
...
(function(){
  ...
  let pk=[],tg=[];
  const ap=def=>({x:def.rx*W,y:def.ry*H});const od=()=>({x:W*0.52,y:H*0.50});
  const lerp=(a,b,u)=>a+(b-a)*Math.min(Math.max(u,0),1);const ease=x=>x<0.5?2*x*x:-1+(4-2*x)*x;
...
@keyframes scanLine2{0%{transform:translateY(-100%);}100%{transform:translateY(400px);}}
@keyframes fileSlide{0%{transform:translateX(-30px);opacity:0;}100%{transform:translateX(0);opacity:1;}}
@keyframes spin{from{transform:rotate(0deg);}to{transform:rotate(360deg);}}
@keyframes notifPop{0%{transform:translateY(20px);opacity:0;}100%{transform:translateY(0);opacity:1;}}
@keyframes rowGlow{0%,100%{background:rgba(0,188,212,0.03);}50%{background:rgba(0,188,212,0.09);}}
@keyframes shimmer{0%{background-position:-200% center;}100%{background-position:200% center;}}
</style>

<div style="font-family:'Inter','Sarabun',sans-serif;font-size:0.6em;">

<!-- Tabs -->
<div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0.5em;margin-bottom:0.7em;">
  <div id="btn-mock-1" onclick="changeMockup(1)" style="cursor:pointer;border-radius:9px;padding:0.55em;text-align:center;border:2px solid #7C4DFF;background:linear-gradient(135deg,rgba(124,77,255,0.25),rgba(124,77,255,0.1));transition:0.3s;">
    <div style="font-size:1.3em;margin-bottom:0.1em;">🤖</div>
    <strong style="color:#7C4DFF;font-size:1em;">1. AI Auto-Labelling</strong>
    <div style="color:#8899aa;font-size:0.8em;margin-top:0.1em;">2 Agents ทำงานพร้อมกัน</div>
  </div>
  <div id="btn-mock-2" onclick="changeMockup(2)" style="cursor:pointer;border-radius:9px;padding:0.55em;text-align:center;border:2px solid transparent;background:rgba(255,255,255,0.04);transition:0.3s;">
    <div style="font-size:1.3em;margin-bottom:0.1em;">⏳</div>
    <strong style="color:#1E88E5;font-size:1em;">2. Lifecycle Policy</strong>
    <div style="color:#8899aa;font-size:0.8em;margin-top:0.1em;">Rule Engine + Data Owner</div>
  </div>
  <div id="btn-mock-3" onclick="changeMockup(3)" style="cursor:pointer;border-radius:9px;padding:0.55em;text-align:center;border:2px solid transparent;background:rgba(255,255,255,0.04);transition:0.3s;">
    <div style="font-size:1.3em;margin-bottom:0.1em;">📋</div>
    <strong style="color:#00BCD4;font-size:1em;">3. Data Catalog</strong>
    <div style="color:#8899aa;font-size:0.8em;margin-top:0.1em;">ETDA Assets + Metadata</div>
  </div>
</div>

<div style="border:1px solid rgba(0,188,212,0.18);border-radius:12px;background:#080f20;height:430px;position:relative;overflow:hidden;box-shadow:0 8px 32px rgba(0,0,0,0.6);">

<!-- ══ PANEL 1 ══ -->
<div id="panel-mock-1" style="display:block;padding:1em;height:100%;box-sizing:border-box;">
  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:0.7em;">
    <div style="font-size:1.05em;font-weight:700;color:#FFF;">🤖 AI Auto-Labelling — 2 Agents ทำงานพร้อมกัน</div>
    <span style="background:rgba(0,230,118,0.15);border:1px solid #00E676;color:#00E676;padding:0.15em 0.7em;border-radius:20px;font-weight:700;font-size:0.8em;animation:agentPulse 2s infinite;">● LIVE</span>
  </div>
  <div style="display:grid;grid-template-columns:1.2fr 1fr 1fr;gap:0.8em;height:88%;">

    <!-- File Queue -->
    <div style="background:rgba(6,14,30,0.8);border:1px solid rgba(255,255,255,0.07);border-radius:9px;padding:0.7em;display:flex;flex-direction:column;gap:0.3em;overflow:hidden;">
      <div style="color:#456070;font-size:0.72em;font-weight:700;margin-bottom:0.15em;flex:0 0 auto;">📂 ไฟล์เข้าคิววิเคราะห์</div>
      <div id="file-queue" style="display:flex;flex-direction:column;gap:0.25em;overflow:hidden;flex:1;"></div>
    </div>

    <!-- Agent 1 -->
    <div style="background:rgba(6,14,30,0.8);border:1.5px solid rgba(0,188,212,0.4);border-radius:9px;padding:0.7em;display:flex;flex-direction:column;animation:agentPulse 3s ease-in-out infinite;position:relative;overflow:hidden;">
      ...
      <div style="display:flex;align-items:center;gap:0.45em;margin-bottom:0.45em;flex:0 0 auto;">
        <div style="position:relative;width:30px;height:30px;">
          ...
          <div style="width:30px;height:30px;background:rgba(0,188,212,0.12);border-radius:50%;border:1px solid #00BCD4;display:flex;align-items:center;justify-content:center;font-size:0.95em;">🔒</div>
        </div>
        <div><div style="color:#00BCD4;font-weight:800;font-size:0.8em;">Classification Agent</div><div style="color:#456070;font-size:0.6em;">ชั้นความลับข้อมูล</div></div>
      </div>
      <div style="color:#607080;font-size:0.65em;margin-bottom:0.2em;flex:0 0 auto;">กำลังวิเคราะห์:</div>
      <div id="cls-current" style="background:rgba(0,188,212,0.07);border:1px solid rgba(0,188,212,0.2);border-radius:5px;padding:0.25em 0.4em;font-size:0.65em;color:#A0B4C8;margin-bottom:0.35em;flex:0 0 auto;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"></div>
      <div style="display:flex;flex-direction:column;gap:0.15em;flex:1;">
        <div style="display:flex;justify-content:space-between;align-items:center;background:rgba(239,83,80,0.07);border-radius:4px;padding:0.18em 0.38em;">
          <span style="color:#EF5350;font-size:0.65em;">🔴 ลับมาก</span><span id="cls-r-n" style="color:#EF5350;font-weight:800;font-size:0.7em;">0</span>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;background:rgba(255,167,38,0.07);border-radius:4px;padding:0.18em 0.38em;">
          <span style="color:#FFA726;font-size:0.65em;">🟡 ภายใน</span><span id="cls-i-n" style="color:#FFA726;font-weight:800;font-size:0.7em;">0</span>
        </div>
        <div style="display:flex;justify-content:space-between;align-items:center;background:rgba(0,188,212,0.07);border-radius:4px;padding:0.18em 0.38em;">
          <span style="color:#00BCD4;font-size:0.65em;">🔵 ทั่วไป</span><span id="cls-p-n" style="color:#00BCD4;font-weight:800;font-size:0.7em;">0</span>
        </div>
        <div style="margin-top:auto;background:rgba(0,230,118,0.07);border:1px solid rgba(0,230,118,0.2);border-radius:6px;padding:0.3em;text-align:center;">
          <div style="font-size:1.4em;font-weight:800;color:#00E676;">94.2%</div>
          <div style="font-size:0.58em;color:#506070;">NLP Accuracy</div>
        </div>
      </div>
    </div>

    <!-- Agent 2 -->
    <div style="background:rgba(6,14,30,0.8);border:1.5px solid rgba(124,77,255,0.4);border-radius:9px;padding:0.7em;display:flex;flex-direction:column;animation:agentPulse2 3s ease-in-out 1s infinite;position:relative;overflow:hidden;">
      <div style="position:absolute;top:0;left:0;right:0;height:1.5px;background:linear-gradient(90deg,transparent,#7C4DFF,transparent);animation:scanLine2 2.5s linear 0.5s infinite;"></div>
      <div style="display:flex;align-items:center;gap:0.45em;margin-bottom:0.45em;flex:0 0 auto;">
        <div style="position:relative;width:30px;height:30px;">
          ...
          <div style="width:30px;height:30px;background:rgba(124,77,255,0.12);border-radius:50%;border:1px solid #7C4DFF;display:flex;align-items:center;justify-content:center;font-size:0.95em;">🏷️</div>
        </div>
        <div><div style="color:#7C4DFF;font-weight:800;font-size:0.8em;">DocType Agent</div><div style="color:#456070;font-size:0.6em;">ประเภทเอกสาร</div></div>
      </div>
      <div style="color:#607080;font-size:0.65em;margin-bottom:0.18em;flex:0 0 auto;">ประเภทที่ตรวจพบ:</div>
      <div style="display:flex;flex-direction:column;gap:0.13em;flex:1;">
        <div style="display:flex;justify-content:space-between;align-items:center;background:rgba(255,179,0,0.06);border-radius:4px;padding:0.16em 0.35em;"><span style="color:#FFB300;font-size:0.62em;">🤝 ประชุมบอร์ด</span><span id="dt-b-n" style="color:#FFB300;font-weight:800;font-size:0.67em;">0</span></div>
        <div style="display:flex;justify-content:space-between;align-items:center;background:rgba(30,136,229,0.06);border-radius:4px;padding:0.16em 0.35em;"><span style="color:#64B5F6;font-size:0.62em;">📝 TOR</span><span id="dt-t-n" style="color:#64B5F6;font-weight:800;font-size:0.67em;">0</span></div>
        <div style="display:flex;justify-content:space-between;align-items:center;background:rgba(0,230,118,0.06);border-radius:4px;padding:0.16em 0.35em;"><span style="color:#00E676;font-size:0.62em;">💵 ใบเสร็จ</span><span id="dt-r-n" style="color:#00E676;font-weight:800;font-size:0.67em;">0</span></div>
        <div style="display:flex;justify-content:space-between;align-items:center;background:rgba(124,77,255,0.06);border-radius:4px;padding:0.16em 0.35em;"><span style="color:#9B72FF;font-size:0.62em;">👥 เอกสาร HR</span><span id="dt-h-n" style="color:#9B72FF;font-weight:800;font-size:0.67em;">0</span></div>
        <div style="display:flex;justify-content:space-between;align-items:center;background:rgba(255,255,255,0.04);border-radius:4px;padding:0.16em 0.35em;"><span style="color:#78909C;font-size:0.62em;">📄 อื่นๆ</span><span id="dt-o-n" style="color:#78909C;font-weight:800;font-size:0.67em;">0</span></div>
        <div style="margin-top:auto;background:rgba(124,77,255,0.07);border:1px solid rgba(124,77,255,0.2);border-radius:6px;padding:0.3em;text-align:center;">
          <div style="font-size:1.4em;font-weight:800;color:#9B72FF;">91.8%</div>
...
<!-- ══ PANEL 2 ══ -->
<div id="panel-mock-2" style="display:none;padding:1em;height:100%;box-sizing:border-box;">
  <div style="display:grid;grid-template-columns:1.3fr 1fr;gap:0.8em;height:100%;">
    <div style="display:flex;flex-direction:column;gap:0.38em;overflow:hidden;">
      <div style="display:flex;justify-content:space-between;align-items:center;flex:0 0 auto;">
        <div style="font-size:1em;font-weight:700;color:#FFF;">⚙️ Rule Engine — ตรวจเงื่อนไขอัตโนมัติ</div>
        <div style="display:flex;align-items:center;gap:0.3em;">
          <div style="width:7px;height:7px;background:#00E676;border-radius:50%;animation:agentPulse 1.5s infinite;"></div>
          <span style="color:#00E676;font-size:0.68em;font-weight:700;">SCANNING</span>
        </div>
      </div>
      <div style="color:#456070;font-size:0.68em;flex:0 0 auto;">🤖 Bot วนตรวจทุกไฟล์ · พบเงื่อนไข → ส่ง Notification ถาม Data Owner</div>
      <div style="display:flex;flex-direction:column;gap:0.2em;flex:1;overflow:hidden;">
        <div style="background:rgba(239,83,80,0.06);border:1px solid rgba(239,83,80,0.18);border-left:3px solid #EF5350;border-radius:7px;padding:0.32em 0.55em;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="color:#EF5350;font-size:0.7em;font-weight:700;">🔴 Confidential + อายุ > 7 ปี</span>
            <span style="font-size:0.62em;color:#00E676;background:rgba(0,230,118,0.1);padding:0.08em 0.35em;border-radius:10px;">● Active</span>
          </div>
          <div style="color:#506070;font-size:0.6em;margin-top:0.08em;">→ แจ้ง CISO + ขอ Approve Archive</div>
          <div style="color:#78909C;font-size:0.58em;margin-top:0.1em;">พบแล้ว: <span id="r1-found" style="color:#EF5350;font-weight:700;">0</span> ไฟล์</div>
        </div>
        <div style="background:rgba(255,167,38,0.05);border:1px solid rgba(255,167,38,0.18);border-left:3px solid #FFA726;border-radius:7px;padding:0.32em 0.55em;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="color:#FFA726;font-size:0.7em;font-weight:700;">🟡 Internal + อายุ > 3 ปี + ไม่ถูกเปิด</span>
            <span style="font-size:0.62em;color:#00E676;background:rgba(0,230,118,0.1);padding:0.08em 0.35em;border-radius:10px;">● Active</span>
          </div>
          <div style="color:#506070;font-size:0.6em;margin-top:0.08em;">→ ถาม Data Owner: Archive หรือยังใช้งาน?</div>
          <div style="color:#78909C;font-size:0.58em;margin-top:0.1em;">พบแล้ว: <span id="r2-found" style="color:#FFA726;font-weight:700;">0</span> ไฟล์</div>
        </div>
        <div style="background:rgba(124,77,255,0.05);border:1px solid rgba(124,77,255,0.18);border-left:3px solid #7C4DFF;border-radius:7px;padding:0.32em 0.55em;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="color:#9B72FF;font-size:0.7em;font-weight:700;">⚠️ PII ไม่มี Encrypt</span>
            <span style="font-size:0.62em;color:#00E676;background:rgba(0,230,118,0.1);padding:0.08em 0.35em;border-radius:10px;">● Active</span>
          </div>
          <div style="color:#506070;font-size:0.6em;margin-top:0.08em;">→ Force Encrypt + แจ้งเตือน Data Owner ทันที</div>
          <div style="color:#78909C;font-size:0.58em;margin-top:0.1em;">พบแล้ว: <span id="r3-found" style="color:#7C4DFF;font-weight:700;">0</span> ไฟล์</div>
        </div>
        <div style="background:rgba(0,188,212,0.05);border:1px solid rgba(0,188,212,0.18);border-left:3px solid #00BCD4;border-radius:7px;padding:0.32em 0.55em;">
          <div style="display:flex;justify-content:space-between;align-items:center;">
            <span style="color:#00BCD4;font-size:0.7em;font-weight:700;">🔵 Public + อายุ > 5 ปี</span>
            <span style="font-size:0.62em;color:#00E676;background:rgba(0,230,118,0.1);padding:0.08em 0.35em;border-radius:10px;">● Active</span>
          </div>
          <div style="color:#506070;font-size:0.6em;margin-top:0.08em;">→ แจ้งเตือน + รอ Approve ก่อน Delete</div>
          <div style="color:#78909C;font-size:0.58em;margin-top:0.1em;">พบแล้ว: <span id="r4-found" style="color:#00BCD4;font-weight:700;">0</span> ไฟล์</div>
        </div>
      </div>
    </div>
    <div style="display:flex;flex-direction:column;gap:0.32em;overflow:hidden;">
      <div style="font-size:0.88em;font-weight:700;color:#FFF;flex:0 0 auto;">📬 แจ้งเตือน Data Owner</div>
      <div style="flex:1;background:rgba(6,14,30,0.8);border:1px solid rgba(255,255,255,0.07);border-radius:9px;padding:0.55em;overflow:hidden;">
        <div id="notif-feed" style="display:flex;flex-direction:column;gap:0.28em;"></div>
      </div>
      <div style="background:rgba(0,230,118,0.06);border:1px solid rgba(0,230,118,0.18);border-radius:8px;padding:0.38em 0.55em;flex:0 0 auto;">
        <div style="font-size:0.65em;color:#607080;margin-bottom:0.12em;">สรุปวันนี้</div>
        <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:0.3em;text-align:center;">
          <div><div style="font-size:1.15em;font-weight:800;color:#FFA726;" id="sum-pending">0</div><div style="font-size:0.58em;color:#607080;">รอ Approve</div></div>
          <div><div style="font-size:1.15em;font-weight:800;color:#00E676;" id="sum-approved">0</div><div style="font-size:0.58em;color:#607080;">Approved</div></div>
          <div><div style="font-size:1.15em;font-weight:800;color:#EF5350;" id="sum-action">0</div><div style="font-size:0.58em;color:#607080;">ดำเนินการแล้ว</div></div>
        </div>
      </div>
    </div>
  </div>
...
<!-- ══ PANEL 3: DATA CATALOG (REDESIGNED) ══ -->
<div id="panel-mock-3" style="display:none;padding:0.9em;height:100%;box-sizing:border-box;flex-direction:column;gap:0.45em;">

  <!-- Header -->
  <div style="display:flex;justify-content:space-between;align-items:center;flex:0 0 auto;">
    <div style="display:flex;align-items:center;gap:0.5em;">
      <div style="width:28px;height:28px;background:linear-gradient(135deg,#00BCD4,#0288D1);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:1em;box-shadow:0 3px 10px rgba(0,188,212,0.4);">🗂️</div>
      <div>
        <div style="font-size:1em;font-weight:800;color:#FFF;">ETDA Data Catalog</div>
        <div style="font-size:0.6em;color:rgba(0,188,212,0.6);letter-spacing:0.04em;">ทะเบียนข้อมูลกลางองค์กร · Governed by AI</div>
      </div>
    </div>
    <div style="display:flex;gap:0.38em;align-items:center;">
      <div style="position:relative;">
        <input type="text" placeholder="🔍 ค้นหา asset..." style="width:170px;padding:0.28em 0.65em;border-radius:15px;border:1px solid rgba(0,188,212,0.3);background:rgba(6,14,30,0.8);color:#FFF;outline:none;font-size:0.68em;">
      </div>
      <select style="background:rgba(6,14,30,0.8);border:1px solid rgba(0,188,212,0.25);border-radius:6px;color:#C0CDD8;padding:0.25em 0.4em;font-size:0.65em;">
        <option>ทุก Label</option><option>🔴 Conf.</option><option>🟡 Internal</option><option>🔵 Public</option>
      </select>
      <select style="background:rgba(6,14,30,0.8);border:1px solid rgba(0,188,212,0.25);border-radius:6px;color:#C0CDD8;padding:0.25em 0.4em;font-size:0.65em;">
        <option>ทุกประเภท</option><option>📊 Dashboard</option><option>📑 Official Doc</option><option>💾 Dataset</option><option>📝 เอกสาร</option>
      </select>
    </div>
  </div>

  <!-- Table -->
  <div style="flex:1;background:rgba(4,10,24,0.9);border:1px solid rgba(0,188,212,0.12);border-radius:10px;overflow:auto;position:relative;">

    <!-- Glow top border -->
    <div style="position:sticky;top:0;z-index:10;background:linear-gradient(90deg,rgba(0,188,212,0.18),rgba(124,77,255,0.12),rgba(0,188,212,0.18));border-bottom:1px solid rgba(0,188,212,0.2);">
      <div style="display:grid;grid-template-columns:1.8fr 0.7fr 0.9fr 0.9fr 1fr 1.1fr 0.6fr;padding:0.42em 0.6em;gap:0.3em;font-size:0.64em;font-weight:700;color:#00BCD4;letter-spacing:0.03em;">
        <div>Asset Name</div>
        <div>Label</div>
        <div>ประเภท</div>
        <div>Data Owner</div>
    ...
    <div style="display:flex;flex-direction:column;padding:0.2em 0;">

      <!-- Row 1: 1212 Dashboard -->
      <div style="display:grid;grid-template-columns:1.8fr 0.7fr 0.9fr 0.9fr 1fr 1.1fr 0.6fr;padding:0.38em 0.6em;gap:0.3em;font-size:0.64em;border-bottom:1px solid rgba(255,255,255,0.04);align-items:center;transition:background 0.2s;" onmouseover="this.style.background='rgba(0,188,212,0.06)'" onmouseout="this.style.background='transparent'">
        <div style="display:flex;align-items:center;gap:0.35em;overflow:hidden;">
          <span style="font-size:1.1em;">📊</span>
          <span style="color:#FFF;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">1212 Complaint Dashboard</span>
        </div>
        <div><span style="background:rgba(255,167,38,0.15);color:#FFA726;border:1px solid rgba(255,167,38,0.35);padding:0.1em 0.4em;border-radius:10px;font-size:0.88em;white-space:nowrap;">🟡 Internal</span></div>
        <div><span style="background:rgba(30,136,229,0.12);color:#64B5F6;padding:0.1em 0.4em;border-radius:5px;font-size:0.88em;">📊 Dashboard</span></div>
        <div style="color:#00BCD4;white-space:nowrap;">ทีม DAC</div>
        <div style="color:#506070;font-size:0.9em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">Power BI Workspace</div>
        <div style="color:#607080;font-size:0.88em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">Source: 1212.csv · Daily refresh</div>
        <div style="text-align:center;"><span style="color:#00E676;font-size:1.1em;">✅</span></div>
      </div>

      <!-- Row 2: ThaiD Dashboard -->
      <div style="display:grid;grid-template-columns:1.8fr 0.7fr 0.9fr 0.9fr 1fr 1.1fr 0.6fr;padding:0.38em 0.6em;gap:0.3em;font-size:0.64em;border-bottom:1px solid rgba(255,255,255,0.04);align-items:center;background:rgba(239,83,80,0.02);transition:background 0.2s;" onmouseover="this.style.background='rgba(239,83,80,0.07)'" onmouseout="this.style.background='rgba(239,83,80,0.02)'">
        <div style="display:flex;align-items:center;gap:0.35em;overflow:hidden;">
          <span style="font-size:1.1em;">📊</span>
          <span style="color:#FFF;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">ThaiD Usage Dashboard</span>
        </div>
        <div><span style="background:rgba(239,83,80,0.15);color:#EF5350;border:1px solid rgba(239,83,80,0.35);padding:0.1em 0.4em;border-radius:10px;font-size:0.88em;white-space:nowrap;">🔴 ลับมาก</span></div>
        <div><span style="background:rgba(30,136,229,0.12);color:#64B5F6;padding:0.1em 0.4em;border-radius:5px;font-size:0.88em;">📊 Dashboard</span></div>
        <div style="color:#00BCD4;white-space:nowrap;">ทีม DID</div>
        <div style="color:#506070;font-size:0.9em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">Power BI Workspace</div>
        <div style="display:flex;gap:0.25em;flex-wrap:wrap;"><span style="background:rgba(255,179,0,0.15);color:#FFB300;padding:0.05em 0.3em;border-radius:3px;font-size:0.82em;">⚠️ PII</span><span style="color:#607080;font-size:0.82em;">Monthly refresh</span></div>
        <div style="text-align:center;"><span style="color:#00E676;font-size:1.1em;">✅</span></div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1.8fr 0.7fr 0.9fr 0.9fr 1fr 1.1fr 0.6fr;padding:0.38em 0.6em;gap:0.3em;font-size:0.64em;border-bottom:1px solid rgba(255,255,255,0.04);align-items:center;transition:background 0.2s;" onmouseover="this.style.background='rgba(0,188,212,0.06)'" onmouseout="this.style.background='transparent'">
        <div style="display:flex;align-items:center;gap:0.35em;overflow:hidden;">
          <span style="font-size:1.1em;">📑</span>
          <span style="color:#FFF;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">ETDA Annual Report 2567</span>
        </div>
        <div><span style="background:rgba(0,188,212,0.12);color:#00BCD4;border:1px solid rgba(0,188,212,0.3);padding:0.1em 0.4em;border-radius:10px;font-size:0.88em;white-space:nowrap;">🔵 ทั่วไป</span></div>
        <div><span style="background:rgba(0,230,118,0.1);color:#00E676;padding:0.1em 0.4em;border-radius:5px;font-size:0.88em;">📑 Official Doc</span></div>
        <div style="color:#00BCD4;white-space:nowrap;">ฝ่ายสื่อสาร</div>
        <div style="color:#506070;font-size:0.9em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">OneDrive/Publications/</div>
        <div style="color:#607080;font-size:0.88em;white-space:nowrap;">PDF · 8.2MB · เผยแพร่สาธารณะ</div>
        <div style="text-align:center;"><span style="color:#00E676;font-size:1.1em;">✅</span></div>
      </div>

      <!-- Row 4: รายงาน DPS -->
      <div style="display:grid;grid-template-columns:1.8fr 0.7fr 0.9fr 0.9fr 1fr 1.1fr 0.6fr;padding:0.38em 0.6em;gap:0.3em;font-size:0.64em;border-bottom:1px solid rgba(255,255,255,0.04);align-items:center;background:rgba(255,167,38,0.02);transition:background 0.2s;" onmouseover="this.style.background='rgba(255,167,38,0.07)'" onmouseout="this.style.background='rgba(255,167,38,0.02)'">
        <div style="display:flex;align-items:center;gap:0.35em;overflow:hidden;">
          <span style="font-size:1.1em;">📑</span>
          <span style="color:#FFF;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">รายงานประจำปี DPS 2567</span>
        </div>
        <div><span style="background:rgba(255,167,38,0.15);color:#FFA726;border:1px solid rgba(255,167,38,0.35);padding:0.1em 0.4em;border-radius:10px;font-size:0.88em;white-space:nowrap;">🟡 Internal</span></div>
        <div><span style="background:rgba(0,230,118,0.1);color:#00E676;padding:0.1em 0.4em;border-radius:5px;font-size:0.88em;">📑 Official Doc</span></div>
        <div style="color:#00BCD4;white-space:nowrap;">ทีม DPS</div>
        <div style="color:#506070;font-size:0.9em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">OneDrive/DPS/Reports/</div>
        <div style="color:#607080;font-size:0.88em;white-space:nowrap;">PDF · 4.5MB · Retention 5 ปี</div>
        <div style="text-align:center;"><span style="color:#00E676;font-size:1.1em;">✅</span></div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1.8fr 0.7fr 0.9fr 0.9fr 1fr 1.1fr 0.6fr;padding:0.38em 0.6em;gap:0.3em;font-size:0.64em;border-bottom:1px solid rgba(255,255,255,0.04);align-items:center;background:rgba(239,83,80,0.02);transition:background 0.2s;" onmouseover="this.style.background='rgba(239,83,80,0.07)'" onmouseout="this.style.background='rgba(239,83,80,0.02)'">
        <div style="display:flex;align-items:center;gap:0.35em;overflow:hidden;">
          <span style="font-size:1.1em;">💾</span>
          <span style="color:#FFF;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">ecommerce_data_2025.csv</span>
        </div>
        <div><span style="background:rgba(239,83,80,0.15);color:#EF5350;border:1px solid rgba(239,83,80,0.35);padding:0.1em 0.4em;border-radius:10px;font-size:0.88em;white-space:nowrap;">🔴 ลับมาก</span></div>
        <div><span style="background:rgba(124,77,255,0.12);color:#9B72FF;padding:0.1em 0.4em;border-radius:5px;font-size:0.88em;">💾 Dataset</span></div>
        <div style="color:#00BCD4;white-space:nowrap;">ทีม DAC</div>
        <div style="color:#506070;font-size:0.9em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">OneDrive/DAC/ecom/</div>
        <div style="display:flex;gap:0.25em;"><span style="background:rgba(255,179,0,0.15);color:#FFB300;padding:0.05em 0.3em;border-radius:3px;font-size:0.82em;">⚠️ PII</span><span style="color:#607080;font-size:0.82em;">2.3MB</span></div>
        <div style="text-align:center;"><span style="color:#00E676;font-size:1.1em;">✅</span></div>
      </div>
      ...
      <div style="display:grid;grid-template-columns:1.8fr 0.7fr 0.9fr 0.9fr 1fr 1.1fr 0.6fr;padding:0.38em 0.6em;gap:0.3em;font-size:0.64em;border-bottom:1px solid rgba(255,255,255,0.04);align-items:center;transition:background 0.2s;" onmouseover="this.style.background='rgba(0,188,212,0.06)'" onmouseout="this.style.background='transparent'">
        <div style="display:flex;align-items:center;gap:0.35em;overflow:hidden;">
          <span style="font-size:1.1em;">📝</span>
          <span style="color:#FFF;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">TOR_จ้างพัฒนาระบบ_AI_2025.pdf</span>
        </div>
        <div><span style="background:rgba(255,167,38,0.15);color:#FFA726;border:1px solid rgba(255,167,38,0.35);padding:0.1em 0.4em;border-radius:10px;font-size:0.88em;white-space:nowrap;">🟡 Internal</span></div>
        <div><span style="background:rgba(255,179,0,0.1);color:#FFB300;padding:0.1em 0.4em;border-radius:5px;font-size:0.88em;">📝 เอกสาร</span></div>
        <div style="color:#00BCD4;white-space:nowrap;">ฝ่ายจัดซื้อ</div>
        <div style="color:#506070;font-size:0.9em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">OneDrive/Procurement/</div>
        <div style="color:#607080;font-size:0.88em;white-space:nowrap;">234KB · มี.ค. 68</div>
        <div style="text-align:center;"><span style="color:#00E676;font-size:1.1em;">✅</span></div>
      </div>

      <!-- Row 7: Server Config (No Policy) -->
      <div style="display:grid;grid-template-columns:1.8fr 0.7fr 0.9fr 0.9fr 1fr 1.1fr 0.6fr;padding:0.38em 0.6em;gap:0.3em;font-size:0.64em;align-items:center;background:rgba(239,83,80,0.04);transition:background 0.2s;" onmouseover="this.style.background='rgba(239,83,80,0.1)'" onmouseout="this.style.background='rgba(239,83,80,0.04)'">
        <div style="display:flex;align-items:center;gap:0.35em;overflow:hidden;">
          <span style="font-size:1.1em;">🔑</span>
          <span style="color:#EF8080;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">server_config_backup_2022/</span>
        </div>
        <div><span style="background:rgba(255,255,255,0.08);color:#78909C;padding:0.1em 0.4em;border-radius:10px;font-size:0.88em;white-space:nowrap;">⚪ ไม่ระบุ</span></div>
        <div><span style="background:rgba(239,83,80,0.1);color:#EF5350;padding:0.1em 0.4em;border-radius:5px;font-size:0.88em;">🔑 Credential?</span></div>
        <div style="color:#EF5350;white-space:nowrap;">— ไม่ระบุ</div>
        <div style="color:#506070;font-size:0.9em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">OneDrive/IT/backup/</div>
        <div style="color:#EF5350;font-size:0.88em;white-space:nowrap;">🚨 ไม่มี Policy</div>
        <div style="text-align:center;"><span style="color:#EF5350;font-size:1.1em;">❌</span></div>
      </div>
  ...
  <!-- Footer legend -->
  <div style="display:flex;justify-content:space-between;align-items:center;flex:0 0 auto;padding:0 0.2em;">
    <div style="display:flex;gap:0.8em;font-size:0.6em;color:#506070;">
      <span><span style="background:rgba(239,83,80,0.15);color:#EF5350;padding:0.05em 0.35em;border-radius:8px;">🔴</span> ลับมาก</span>
      <span><span style="background:rgba(255,167,38,0.15);color:#FFA726;padding:0.05em 0.35em;border-radius:8px;">🟡</span> ภายใน</span>
      <span><span style="background:rgba(0,188,212,0.12);color:#00BCD4;padding:0.05em 0.35em;border-radius:8px;">🔵</span> ทั่วไป</span>
    </div>
    <div style="font-size:0.58em;color:#3A5060;">📍 Source: Microsoft Graph API · Extracted: Apr 2026 · Confidential — Internal Use Only</div>
  </div>
...
<script>
const mc={1:{col:'#7C4DFF',rgb:'124,77,255'},2:{col:'#1E88E5',rgb:'30,136,229'},3:{col:'#00BCD4',rgb:'0,188,212'}};
function changeMockup(n){
  for(let i=1;i<=3;i++){
    document.getElementById('panel-mock-'+i).style.display='none';
    const b=document.getElementById('btn-mock-'+i);
    b.style.border='2px solid transparent';b.style.background='rgba(255,255,255,0.04)';
  }
  const p=document.getElementById('panel-mock-'+n);
  p.style.display=(n===3)?'flex':'block';
  const b=document.getElementById('btn-mock-'+n);const c=mc[n];
  b.style.border='2px solid '+c.col;
  b.style.background='linear-gradient(135deg,rgba('+c.rgb+',0.25),rgba('+c.rgb+',0.1))';
}
document.addEventListener('DOMContentLoaded',()=>{changeMockup(1);startP1();startP2();});

function startP1(){
  const files=[{name:'Budget_Q4_2025.xlsx',icon:'📊'},{name:'MOU_Digital_ID.pdf',icon:'📄'},{name:'Staff_2025.csv',icon:'📋'},{name:'TOR_จ้างพัฒนา.pdf',icon:'📑'},{name:'1212_Q1_Complaint.csv',icon:'💬'},{name:'ใบเสร็จ_2025_001.pdf',icon:'💵'},{name:'Board_Meeting_Mar.docx',icon:'🤝'},{name:'HR_Contract_2025.pdf',icon:'👥'}];
  const clsLabels=['🔴','🟡','🟡','🟡','🔴','🔵','🟡','🔴'];
  const dtLabels=['💵','📝','👥','🤝','💵','🤝','🤝','👥'];
  let fi=0,cn={r:0,i:0,p:0},dt={b:0,t:0,r:0,h:0,o:0};
  const q=document.getElementById('file-queue');
  function addFile(){
    const f=files[fi%files.length];
    const div=document.createElement('div');
    div.style.cssText='display:flex;align-items:center;gap:0.3em;background:rgba(0,188,212,0.06);border:1px solid rgba(0,188,212,0.14);border-radius:4px;padding:0.16em 0.32em;animation:fileSlide 0.4s ease;flex-shrink:0;';
    div.innerHTML=`<span style="font-size:0.88em;">${f.icon}</span><span style="font-size:0.62em;color:#A0B4C8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${f.name}</span>`;
    q.appendChild(div);if(q.children.length>6)q.removeChild(q.firstChild);
    document.getElementById('cls-current').textContent=f.icon+' '+f.name;
    const cl=clsLabels[fi%files.length];
    if(cl==='🔴')cn.r++;else if(cl==='🟡')cn.i++;else cn.p++;
    document.getElementById('cls-r-n').textContent=cn.r;
    document.getElementById('cls-i-n').textContent=cn.i;
    document.getElementById('cls-p-n').textContent=cn.p;
    const dl=dtLabels[fi%files.length];
    if(dl==='🤝')dt.b++;else if(dl==='📝')dt.t++;else if(dl==='💵')dt.r++;else if(dl==='👥')dt.h++;else dt.o++;
    document.getElementById('dt-b-n').textContent=dt.b;
    document.getElementById('dt-t-n').textContent=dt.t;
    document.getElementById('dt-r-n').textContent=dt.r;
    document.getElementById('dt-h-n').textContent=dt.h;
    document.getElementById('dt-o-n').textContent=dt.o;
    fi++;
  }
  addFile();setInterval(addFile,1800);
}

function startP2(){
  let r1=0,r2=0,r3=0,r4=0,pending=0,approved=0,actioned=0;
  const notifs=[
    {msg:'📦 Budget_2018.xlsx ครบ 7 ปี — ขอ Approve Archive',from:'CISO',col:'#EF5350',rule:1},
    {msg:'🟡 Report_2021.pdf ไม่ถูกเปิด 3 ปี — ยังใช้งานอยู่?',from:'DAC Team',col:'#FFA726',rule:2},
    {msg:'⚠️ Staff_List.csv ไม่มี Encrypt — กรุณา Action!',from:'HR Owner',col:'#7C4DFF',rule:3},
    {msg:'📦 MOU_2019.docx ครบ 7 ปี — ขอ Approve Archive',from:'Legal',col:'#EF5350',rule:1},
    {msg:'🔵 Guideline_2018.pdf ครบ 5 ปี — ยืนยันลบ?',from:'Policy Team',col:'#00BCD4',rule:4},
    {msg:'✅ Budget_2018.xlsx — Approved Archive โดย CISO',from:'CISO',col:'#00E676',approved:true},
    {msg:'🔒 1212_PII.csv — Force Encrypted แล้ว',from:'System',col:'#7C4DFF',rule:3,done:true},
  ];
  let ni=0;
  const feed=document.getElementById('notif-feed');
  function addNotif(){
    const n=notifs[ni%notifs.length];ni++;
    if(n.rule===1)r1++;if(n.rule===2)r2++;if(n.rule===3)r3++;if(n.rule===4)r4++;
    document.getElementById('r1-found').textContent=r1;
    document.getElementById('r2-found').textContent=r2;
    document.getElementById('r3-found').textContent=r3;
    document.getElementById('r4-found').textContent=r4;
    if(n.approved){approved++;actioned++;}else if(n.done){actioned++;}else{pending++;}
    document.getElementById('sum-pending').textContent=pending;
    document.getElementById('sum-approved').textContent=approved;
    document.getElementById('sum-action').textContent=actioned;
    const div=document.createElement('div');
    div.style.cssText=`background:rgba(255,255,255,0.02);border:1px solid ${n.col}30;border-left:2px solid ${n.col};border-radius:6px;padding:0.25em 0.4em;animation:notifPop 0.4s ease;`;
    div.innerHTML=`<div style="font-size:0.63em;color:${n.col};font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${n.msg}</div><div style="font-size:0.57em;color:#456070;margin-top:0.06em;">→ แจ้ง: ${n.from}</div>`;
    feed.insertBefore(div,feed.firstChild);
    if(feed.children.length>5)feed.removeChild(feed.lastChild);
  }
  addNotif();setInterval(addNotif,2500);
}
</script>
```
